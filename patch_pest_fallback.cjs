...
function patchFile(filepath) {
  ...
  // Find the try block for gemini in pest analysis
  // In server.ts:
  ...
  const pestApiRegex = /app\.post\("\/api\/analyze-pest"[\s\S]*?console\.error\("Pest analysis error:", error\);\n\s*return res\.status\(500\)\.json\(\{ error: error\.message \|\| "Failed to analyze pest\/disease" \}\);\n\s*\}\n\}\);/;
  
  if (content.match(pestApiRegex)) {
